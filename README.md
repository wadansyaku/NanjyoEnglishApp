...
### 8) `POST /api/v1/ai/meaning-suggest`
...
### 10) `GET /api/v1/wordbank/decks`

- 出力: `deck` 一覧（タイトル、語数、source）

### 11) `GET /api/v1/wordbank/curriculum`

- 出力: 速習/標準トラックとステップ情報（学習順・語数・推奨区切り）

### 12) `GET /api/v1/wordbank/decks/:deckId/words`

- 出力: 指定デッキの語彙一覧（`headword_norm`, `meaning_ja_short`）

### 13) `POST /api/v1/wordbank/decks/words-batch`

- 入力: `{ deckIds: string[] }`
- 出力: 複数デッキを順序維持で結合し、重複除去した語彙一覧
- 備考: カリキュラム内部では `slice:<deckId>:<start>:<end>` 形式で範囲分割デッキを扱える

### 14) `POST /api/v1/wordbank/admin/upsert-words` (Admin)

- ヘッダ: `x-admin-token` または `Authorization: Bearer <ADMIN_TOKEN>`
- 入力: words/decks のupsert

### 15) `GET /api/v1/admin/students` (Admin)

- 生徒ごとの進捗サマリ（Lv/XP/履修語数/最終同期）を返す

### 16) `GET /api/v1/admin/students/:userId/words` (Admin)

- 指定生徒の履修語（テスト作成元）を返す

### 17) `GET /api/v1/admin/feedback` (Admin)

- 最新のアプリ意見一覧を返す（screen/device などの短い context を含む）

### 18) `GET /api/v1/admin/settings` (Admin)

- 全ユーザー向けの共通設定を返す

### 19) `POST /api/v1/admin/settings` (Admin)

- 全ユーザー向けの共通設定を更新する

### 20) `GET /api/v1/settings/public`

- ログイン不要。全ユーザー向け共通設定を返す

### 21) `POST /api/v1/community/changesets`

- 提案（changeset）を作成

### 22) `POST /api/v1/community/changesets/:id/items`

- 提案に単語差分を追加

### 23) `POST /api/v1/community/changesets/:id/submit`

- `draft -> proposed`

### 24) `POST /api/v1/community/changesets/:id/review`

- `approve / request_changes / comment`

### 25) `POST /api/v1/community/changesets/:id/merge`
